const robotBoton = document.querySelector(".robot-boton");
const robotMensaje = document.querySelector(".robot-mensaje");

const saludarRobot = () => {
  if (!robotBoton || !robotMensaje) return;
  robotBoton.classList.remove("saludando");
  void robotBoton.offsetWidth;
  robotBoton.classList.add("saludando");
  robotMensaje.textContent = "¡Bienvenido a Cipa Software Squad!";
  robotMensaje.classList.add("activo");
  window.clearTimeout(saludarRobot.temporizador);
  saludarRobot.temporizador = window.setTimeout(() => {
    robotMensaje.textContent = "Listo para construir el futuro contigo.";
    robotMensaje.classList.remove("activo");
  }, 3600);
};

robotBoton?.addEventListener("click", saludarRobot);

robotBoton?.addEventListener("pointermove", (evento) => {
  if (evento.pointerType === "touch") return;
  const rect = robotBoton.getBoundingClientRect();
  const x = ((evento.clientX - rect.left) / rect.width - .5) * 5;
  const eyes = robotBoton.querySelector("img");
  eyes?.style.setProperty("--mirada-x", `${x}px`);
});

document.addEventListener("visibilitychange", () => {
  document.documentElement.classList.toggle("pestaña-oculta", document.hidden);
});
document.documentElement.classList.add("js");
document.querySelectorAll("[data-edad-nacimiento]").forEach((elemento) => {
  const nacimiento = new Date(`${elemento.dataset.edadNacimiento}T00:00:00`);
  const hoy = new Date();
  let edad = hoy.getFullYear() - nacimiento.getFullYear();
  const aniversarioPendiente = hoy.getMonth() < nacimiento.getMonth() || (hoy.getMonth() === nacimiento.getMonth() && hoy.getDate() < nacimiento.getDate());
  if (aniversarioPendiente) edad -= 1;
  elemento.textContent = `${edad} años`;
});
